...
Designed as an inbox for knowledge-base tools like [Foam](https://foamnotes.com/) and [Obsidian](https://obsidian.md/).
...
`--bookmarks` is optional; the path is auto-detected for Windows, macOS, Linux, and WSL2.

On first run, `sync` also scaffolds the following files if they do not exist:

- `filter.ini.example` — template for filter rules (copy to `filter.ini` to activate)
- `.env.example` — template for fetch credentials
- `.gitignore` — minimal file containing `.env` and `*.db` (only created when none exists)
...
On first run, `filter.ini` is created by copying `filter.ini.example` (generated by `sync`).
...
Summary output shows before/after counts for `PENDING` and `IGNORED`, with `(no change)` when unchanged.
`ERROR` breakdowns are shown only when ERROR articles exist.
...
Run `sync` first to generate `.env.example`, then copy it to `.env` and fill in your credentials:
...
[Foam](https://foamnotes.com/) や [Obsidian](https://obsidian.md/) などのナレッジベースツールのインボックスとして使用します。
...
`--bookmarks` を省略するとWindows・macOS・Linux・WSL2のパスを自動検出。

初回実行時に以下のファイルが存在しない場合は自動生成されます:

- `filter.ini.example` — フィルタルールのテンプレート（`filter.ini` にコピーして使用）
- `.env.example` — fetch認証情報のテンプレート
- `.gitignore` — `.env` と `*.db` を含む最小構成（`.gitignore` が存在しない場合のみ作成）
...
初回実行時に `filter.ini.example`（`sync` 実行時に生成）から `filter.ini` が自動生成されます。
...
実行結果には `PENDING`・`IGNORED` の変更前後の件数が表示されます（変化なしの場合は `(no change)` を付記）。
`ERROR` の内訳は ERROR 件数が存在する場合のみ表示されます。
...
まず `sync` を実行して `.env.example` を生成し、`.env` にコピーして認証情報を設定:
